...
export class SearchResultsList {
  ...
  generateResultsListNode(amount) {
    ...
    for (let i = 0; i < amount; i++) {
      ...
      imageContainerNode.classList.add(
        "search-bar-results-list-result-image-container"
      );
      ...
      textContainerNode.classList.add(
        "search-bar-results-list-result-text-container"
      );
      ...
      textAsideContainerNode.classList.add(
        "search-bar-results-list-result-text-aside-container"
      );
      ...
      priceChangesNode.classList.add(
        "search-bar-results-list-result-price-changes"
      );
      ...
      compareButtonNode.classList.add(
        "search-bar-results-list-result-compare-button"
      );
      compareButtonNode.type = "submit";
      compareButtonNode.textContent = "Compare";
      compareButtonNode.addEventListener(
        "click",
        this.compareButtonClick.bind(this)
      );
  ...
  hideResults(startIndex) {
    ...
    if (startIndex == 0) this.rootNode.classList.add("d-none");
  }
  showResults(endIndex) {
    if (endIndex > 0) this.rootNode.classList.remove("d-none");
  ...
  findSequencesOfText(query, text, isCaseSensitive) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (letter != query[activeSequence.length]) {
        if (activeSequence.length > 0) activeSequence = [];
      }
      if (letter == query[activeSequence.length]) {
        ...
        if (activeSequence.length == query.length) {
          foundSequences.push({
            startIndex: activeSequenceStartIndex,
            text: activeSequence,
          });
          activeSequence = [];
        }
  ...
  highlightNodeText(queryToHighlight, targetNode) {
    ...
    const sequencesToHighlight = this.findSequencesOfText(
      queryToHighlight,
      text,
      false
    );

    if (sequencesToHighlight.length == 0) return;
    ...
    for (const sequenceToHighlight of sequencesToHighlight) {
      if (sequenceToHighlight.startIndex > textIndex) {
        const textNode = document.createTextNode(
          text.slice(textIndex, sequenceToHighlight.startIndex)
        );
    ...
    if (textIndex != text.length) {
      const textNode = document.createTextNode(
        text.slice(textIndex, text.length)
      );
      targetNode.appendChild(textNode);
    }
  }
  renderResults(searchQuery, companyProfiles) {
    const endIndex = Math.min(companyProfiles.length, this.resultNodes.length);
    this.showResults(endIndex);
    for (let i = 0; i < endIndex; i++) {
      const symbol = companyProfiles[i].symbol;
      const profile = companyProfiles[i].profile;
      const resultNode = this.resultNodes[i];

      const companyNameNode = resultNode.querySelector(
        ".search-bar-results-list-result-company-name"
      );
      ...
      const symbolNode = resultNode.querySelector(
        ".search-bar-results-list-result-symbol"
      );
      symbolNode.innerHTML = "";
      symbolNode.textContent = `(${symbol})`;
      this.highlightNodeText(searchQuery, symbolNode);

      const linkNode = resultNode.querySelector(
        ".search-bar-results-list-result-link"
      );
      linkNode.href = `${this.companyPageLocalUrl}?symbol=${symbol}`;
      linkNode.title = profile.companyName;
      const priceChangesNode = resultNode.querySelector(
        ".search-bar-results-list-result-price-changes"
      );
      updatePriceChangesNode(priceChangesNode, profile.changesPercentage);

      const imageNode = resultNode.querySelector(
        ".search-bar-results-list-result-image"
      );
      imageNode.src = profile.image;

      const compareButtonNode = resultNode.querySelector(
        ".search-bar-results-list-result-compare-button"
      );
