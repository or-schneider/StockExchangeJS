export class CompanyComparisonList {
  ...
  updateCompareLink(text, link, isEnabled) {
    if (isEnabled)
      this.compareLinkNode.classList.remove(
        "company-comparison-list-compare-link-disable"
      );
    else
      this.compareLinkNode.classList.add(
        "company-comparison-list-compare-link-disable"
      );
  ...
  refreshCompareLink() {
    ...
    if (this.targetsSet.size == 1) text += "company";
    else text += "companies";
  ...
  generate() {
    ...
    this.contentContainerNode.classList.add(
      "company-comparison-list-content-container"
    );

    this.companyComparisonButtonsNode = document.createElement("div");
    this.companyComparisonButtonsNode.classList.add(
      "company-comparison-buttons"
    );
  ...
  add(companyData) {
    if (this.targetsSet.has(companyData)) return;
    if (this.targetsSet.size >= this.maxTargets) return;
