...
export async function fetchCompaniesDataAsync(symbols) {
  ...
  const symbolsChunks = convertSymbolsToChunks(
    companyProfileRequestMaxSymbols,
    symbols
  );
...
function convertSymbolsToChunks(chunkSize, symbols) {
  ...
  let leftOverStartIndex =
    Math.max(chunkSize, symbols.length) -
    (Math.max(chunkSize, symbols.length) % Math.min(chunkSize, symbols.length));
